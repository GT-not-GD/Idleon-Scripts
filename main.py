...
DEBUG_FOLDER = "debug_screenshots" # 调试截图文件夹
...
class GameBot:
    ...
    def __init__(self, window_configs):
        """
        初始化 GameBot 实例
        Args:
            window_configs (dict): 窗口配置字典
        """
        self.hwnds = {} # 存储所有窗口的句柄
        for key, name in window_configs.items():
            hwnd = win32gui.FindWindow(None, name)
            if not hwnd:
                raise Exception(f"未找到窗口 '{name}' (key: {key})")
            self.hwnds[key] = hwnd
            print(f"成功找到窗口 '{name}' (key: {key}), HWND: {hwnd}")

        # 设置默认的活动窗口
        self.active_window_key = 'default'
        ...
        # 为每个窗口创建一个独立的截图缓存
        self.screenshot_caches = {key: None for key in self.hwnds.keys()}

    @property
    def active_hwnd(self):
        """获取当前活动窗口的句柄"""
        return self.hwnds[self.active_window_key]

    @property
    def screenshot_cache(self):
        """获取或设置当前活动窗口的截图缓存"""
        return self.screenshot_caches[self.active_window_key]

    @screenshot_cache.setter
    def screenshot_cache(self, value):
        self.screenshot_caches[self.active_window_key] = value

    @contextmanager
    def target_window(self, key='default'):
        """
        一个上下文管理器，用于临时切换 GameBot 的目标窗口。
        使用方法:
        with bot.target_window('other'):
            # 在此代码块中，所有 bot 操作都将针对 'other' 窗口
            bot.interact_with_image(...)
        # 离开代码块后，bot 会自动恢复到之前的目标窗口
        """
        if key not in self.hwnds:
            raise ValueError(f"窗口 key '{key}' 不存在。可用的 keys: {list(self.hwnds.keys())}")

        original_key = self.active_window_key
        try:
            # 进入 with 代码块前，切换到目标窗口
            self.active_window_key = key
            # print(f"Switched to window: {key}") # 用于调试
            yield
        finally:
            # 离开 with 代码块后，无论是否发生异常，都切换回原始窗口
            self.active_window_key = original_key
            # print(f"Switched back to window: {original_key}") # 用于调试
    ...
    def get_screenshot(self, max_loc=None, x_offset=0, y_offset=0, width=None, height=None, refresh=False):
        ...
        try:
            # 获取窗口矩形坐标
            left, top, right, bottom = win32gui.GetWindowRect(self.active_hwnd)
            if max_loc:
                # 计算目标截图区域
    ...
    def _capture_window(self):
        ...
        try:
            ...
            # 创建设备上下文和位图对象
            hdc = win32gui.GetWindowDC(self.active_hwnd)
            ...
            saveDC.DeleteDC()
            mfcDC.DeleteDC()
            win32gui.ReleaseDC(self.active_hwnd, hdc)
    ...
    def find_image_location(self, image_path, refresh_screenshot=False):
        ...
        Returns:
            tuple: (max_loc, template_shape, screenshot) 如果找到，否则返回 (None, None, None)
                   max_loc: 图像在窗口中的位置 (x, y)
                   template_shape: 模板图像的 (height, width)
                   screenshot: 用于查找的原始截图
        """
        # 获取窗口截图
        ...
        # 如果匹配值超过阈值，返回匹配位置、模板尺寸和截图
        if max_val >= THRESHOLD:
            return max_loc, template.shape, screenshot # 返回更多信息
        return None, None, screenshot # 即使没找到也返回截图，方便调试
    
    def _save_debug_screenshot(self, base_image, found_loc, template_shape, click_point, image_path, action_name):
        """
        保存带有调试信息的截图。

        Args:
            base_image (numpy.ndarray): 用于绘制的基础截图。
            found_loc (tuple): 找到的模板左上角坐标 (x, y)。
            template_shape (tuple): 模板的形状 (height, width)。
            click_point (tuple): 实际点击的坐标 (x, y)。
            image_path (str): 查找的图片路径，用于命名文件。
            action_name (str): 操作名称，如 "interact" 或 "click_hold"。
        """
        ...
        # 如果找到了图片，画出匹配的矩形框
        if found_loc and template_shape:
            top_left = found_loc
        ...
        if click_point:
            # 用红色画一个圆圈标记点击位置
            cv2.circle(debug_image, click_point, 10, (0, 0, 255), 2)
            # 用红色画一个十字准星
            cv2.drawMarker(debug_image, click_point, (0, 0, 255), markerType=cv2.MARKER_CROSS, markerSize=15, thickness=2)

        # 生成唯一的文件名
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        ms = int(time.time() * 1000) % 1000
        template_name = os.path.basename(image_path).split('.')[0] if image_path else "coord_click"
        filename = f"{timestamp}_{ms}_{action_name}_{template_name}.png"
    ...
    def interact_with_image(self, image_path, duration=0, refresh=False, click_times=1, x_offset=0, y_offset=0, delay=0, swipe=False):
        ...
        max_loc, template_shape, screenshot = self.find_image_location(image_path, refresh_screenshot=refresh)
        ...
        if max_loc: # 如果找到图像
            ...
            # --- 保存调试截图 ---
            self._save_debug_screenshot(screenshot, max_loc, template_shape, click_point, image_path, "interact")
            ...
            # --- 保存未找到目标的调试截图 ---
            print(f"  [调试] 未在窗口 '{self.active_window_key}' 中找到图片: {image_path}")
            self._save_debug_screenshot(screenshot, None, None, None, image_path, "interact_FAIL")
            return False
    ...
    def _click_and_hold(self, x, y, duration=0, click_times=1, x_offset=0, y_offset=0):
        ...
        # --- 保存调试截图 ---
        current_screenshot = self.get_screenshot(refresh=True)
        self._save_debug_screenshot(current_screenshot, None, None, click_point, None, "click_hold")

        for _ in range(click_times):
            # 核心：发送一次 LBUTTONDOWN 来“启动”点击状态
            win32api.PostMessage(self.active_hwnd, win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, lParam)
            time.sleep(0.05) # 短暂延迟，确保游戏捕捉到初始按下

            # 如果是长按，进入你的原始循环逻辑
            if duration > 0:
                start_time = time.time()
                # 在持续时间内，可以继续发送 MOUSEMOVE 来模拟按住，这比反复发送 DOWN 更安全
                while time.time() - start_time < duration:
                    # 发送 WM_MOUSEMOVE 消息，并附带 MK_LBUTTON 标志，
                    # 这告诉窗口“鼠标在移动，并且左键仍然是按下的”。
                    # 这种方法通常比反复发送 DOWN 消息更稳定。
                    win32api.PostMessage(self.active_hwnd, win32con.WM_MOUSEMOVE, win32con.MK_LBUTTON, lParam)
                    time.sleep(0.05) # 模拟按住时的帧率
            
            # 最终，发送 LBUTTONUP 来结束点击
            win32api.PostMessage(self.active_hwnd, win32con.WM_LBUTTONUP, 0, lParam)
        
        if click_times > 1:
            time.sleep(0.1) # 多次点击之间的间隔
    ...
    def _swipe_loot(self, center_x, center_y, width, steps):
        ...
        win32api.PostMessage(self.active_hwnd, win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, lParam_start) # 鼠标左键按下

        for i in range(1, steps + 1): # 平滑滑动 steps 步
            x = int(start_x + i * x_step) # 计算当前步 x 坐标
            lParam = win32api.MAKELONG(x, center_y) # 当前位置 lParam
            win32api.PostMessage(self.active_hwnd, win32con.WM_MOUSEMOVE, win32con.MK_LBUTTON, lParam) # 发送鼠标移动消息
            time.sleep(0.01)

        lParam_end = win32api.MAKELONG(end_x, center_y) # 滑动结束位置 lParam
        win32api.PostMessage(self.active_hwnd, win32con.WM_LBUTTONUP, 0, lParam_end) # 鼠标左键释放
    ...
    def press_key(self, key, duration=0):
        ...
        if duration > 0: # 按住一段时间
            start_time = time.time()
            while time.time() - start_time < duration:
                win32api.PostMessage(self.active_hwnd, win32con.WM_KEYDOWN, vk_code, 0) # 按下按键
                time.sleep(0.05)
                win32api.PostMessage(self.active_hwnd, win32con.WM_KEYUP, vk_code, 0) # 释放按键
                time.sleep(0.05)
        else: # 短按一下
            win32api.PostMessage(self.active_hwnd, win32con.WM_KEYDOWN, vk_code, 0) # 按下按键
...
class GameFunctions:
    ...
    def afk_fast_forward_loop(self):
        ...
        try:
            while self.bot.running:
                ...
                time.sleep(0.5)
                self.bot.interact_with_image(self.bot.config["afk"]["candy_image"], duration=0.6) # 使用糖果
                time.sleep(0.9)
                self.bot.interact_with_image(self.bot.config["afk"]["storage_image"]) # 领取奖励
                time.sleep(0.5)
                self.bot.interact_with_image(self.bot.config["afk"]["gt_leg_image"], swipe=False) # 滑动 GT 腿
                time.sleep(0.2)
        except Exception as e:
    ...
    def _setup_fishing_auto(self):
        ...
        with self.bot.target_window('default'):
            try:
                # 激活默认的 Idleon 窗口
    ...
    def test_loop(self):
        ...
        while self.bot.running:
            try:
                with self.bot.target_window('other'):
                    self.bot.interact_with_image(self.bot.config["cheat_ui"]["spawn_green_mushroom"])
                    time.sleep(60) 

            except KeyError:
                print("\n错误：请确保你的 config.json 文件中包含 'other_app' 的配置。")
                print("示例: \"other_app\": { \"some_button_image\": \"path/to/image.png\" }")
                break
            except Exception as e:
                print(f"test_loop 中发生错误: {e}")
                break
...
def main():
    ...
    try:
        # 定义所有需要控制的窗口
        window_configs = {
            'default': 'Legends Of Idleon',
            'other': 'Idleon Cheat Injector UI' 
        }
        # 用配置字典一次性初始化 GameBot
        bot = GameBot(window_configs)
